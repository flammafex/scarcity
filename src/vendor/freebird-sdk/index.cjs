...
__export(src_exports, {
  BatchIssuanceError: () => BatchIssuanceError,
  DiscoveryError: () => DiscoveryError,
  ExchangeError: () => ExchangeError,
  FreebirdClient: () => FreebirdClient,
  FreebirdError: () => FreebirdError,
  GraphIssuanceError: () => GraphIssuanceError,
  InvalidTokenError: () => InvalidTokenError,
  MemoryTokenStore: () => MemoryTokenStore,
  PollAbortedError: () => PollAbortedError,
  PollError: () => PollError,
  PollTimeoutError: () => PollTimeoutError,
  RateLimitedError: () => RateLimitedError,
  ReplayedTokenError: () => ReplayedTokenError,
  StorageTokenStore: () => StorageTokenStore,
  VerificationError: () => VerificationError,
  VerifierNotConfiguredError: () => VerifierNotConfiguredError,
  VerifierUnavailableError: () => VerifierUnavailableError,
  buildBatchBinding: () => buildBatchBinding,
  buildGraphIssuanceHmacAuthorizationV2: () => buildGraphIssuanceHmacAuthorizationV2,
  buildHmacAuthorizationV2: () => buildHmacAuthorizationV2,
  buildIssueBinding: () => buildIssueBinding,
  buildPublicIssueBinding: () => buildPublicIssueBinding,
  buildRenewBinding: () => buildRenewBinding,
  crypto: () => crypto2,
  deserializeGraphIssuanceRecoveryContext: () => deserializeGraphIssuanceRecoveryContext,
  exchangePasses: () => exchangePasses,
  generateOperationId: () => generateOperationId,
  generateProofOfWork: () => generateProofOfWork,
  generateStatusCapability: () => generateStatusCapability,
  graphIssuanceHmacAuthorizationTagV2: () => graphIssuanceHmacAuthorizationTagV2,
  graphIssuanceHmacAuthorizationTranscriptV2: () => graphIssuanceHmacAuthorizationTranscriptV2,
  hmacAuthorizationTagV2: () => hmacAuthorizationTagV2,
  hmacAuthorizationTranscriptV2: () => hmacAuthorizationTranscriptV2,
  parseGraphIssuanceHmacAuthorizationV2: () => parseGraphIssuanceHmacAuthorizationV2,
  parseHmacAuthorizationV2: () => parseHmacAuthorizationV2,
  pollExchangeStatus: () => pollExchangeStatus,
  pollGraphIssuanceStatus: () => pollGraphIssuanceStatus,
  pollUntilTerminal: () => pollUntilTerminal,
  serializeGraphIssuanceRecoveryContext: () => serializeGraphIssuanceRecoveryContext,
  tokenId: () => tokenId,
  verifyGraphIssuanceHmacAuthorizationV2: () => verifyGraphIssuanceHmacAuthorizationV2,
  verifyHmacAuthorizationV2: () => verifyHmacAuthorizationV2,
  verifyPow: () => verifyPow
});
...
var P = import_p256.p256.CURVE.Fp.ORDER;
var N = import_p256.p256.CURVE.n;
var A = import_p256.p256.CURVE.a;
var B = import_p256.p256.CURVE.b;
var Z = BigInt(-10);
...
function randomScalar() {
  const randomBytes2 = import_p256.p256.utils.randomPrivateKey();
...
function decodePoint(bytes) {
  try {
    return import_p256.p256.ProjectivePoint.fromHex((0, import_utils.bytesToHex)(bytes));
  } catch (e) {
...
function os2ip(bytes) {
  return BigInt("0x" + (0, import_utils.bytesToHex)(bytes));
}
...
function expandMessageXMD(msg, dst, lenInBytes) {
  ...
  const dstPrime = (0, import_utils.concatBytes)(dst, new Uint8Array([dst.length]));
  ...
  const msgPrime = (0, import_utils.concatBytes)(Z_pad, msg, l_i_b_str, new Uint8Array([0]), dstPrime);
  let b_0 = (0, import_sha256.sha256)(msgPrime);
  let b_1 = (0, import_sha256.sha256)((0, import_utils.concatBytes)(b_0, new Uint8Array([1]), dstPrime));
  ...
  while (offset < lenInBytes) {
    ...
    b_i = (0, import_sha256.sha256)((0, import_utils.concatBytes)(xorBytes, new Uint8Array([i]), dstPrime));
...
function mapToCurveSSWU(u) {
  ...
  if (y1 !== null) {
    if (y1 % BigInt(2) !== u % BigInt(2))
      y1 = mod(-y1, P);
    return new import_p256.p256.ProjectivePoint(x1, y1, BigInt(1));
  }
  ...
  if (y2 % BigInt(2) !== u % BigInt(2))
    y2 = mod(-y2, P);
  return new import_p256.p256.ProjectivePoint(x2, y2, BigInt(1));
}
function sqrt(x) {
...
function finalize(state, tokenB64, issuerPubkeyB64, context) {
  ...
  const G = import_p2562.p256.ProjectivePoint.BASE;
  ...
  const finalizeInput = (0, import_utils2.concatBytes)(
    new TextEncoder().encode("VOPRF-P256-SHA256:Finalize"),
    context,
    wBytes
  );
  const output = (0, import_sha2562.sha256)(finalizeInput);
  return output;
}
function buildScopeDigest(verifierId, audience) {
  ...
  return (0, import_sha2562.sha256)((0, import_utils2.concatBytes)(
...
function buildPrivateTokenInput(issuerId, kid, nonce, scopeDigest) {
  ...
  return (0, import_utils2.concatBytes)(
...
function tokenKeyIdFromSpki(pubkeySpki) {
  return (0, import_sha2562.sha256)(pubkeySpki);
}
function tokenKeyIdToHex(tokenKeyId) {
  ...
  return (0, import_utils2.bytesToHex)(tokenKeyId);
}
function tokenKeyIdFromHex(tokenKeyIdHex) {
  ...
  return (0, import_utils2.hexToBytes)(tokenKeyIdHex);
}
function buildPublicBearerMessage(nonce, tokenKeyId, issuerId) {
  ...
  return (0, import_sha512.sha384)((0, import_utils2.concatBytes)(
...
function verifyDleq(G, Y, A2, B2, proofBytes, context) {
  ...
  const dst = (0, import_utils2.concatBytes)(DLEQ_DST_PREFIX, context);
  const dstLenBytes = numberToBytesBE(dst.length, 4);
  const transcript = (0, import_utils2.concatBytes)(
    dstLenBytes,
    dst,
    encodePoint(G),
    encodePoint(Y),
...
function bytesToNumber(bytes) {
  return BigInt("0x" + (0, import_utils2.bytesToHex)(bytes));
}
function numberToBytesBE(num, len) {
  const hex2 = num.toString(16).padStart(len * 2, "0");
  return (0, import_utils2.hexToBytes)(hex2);
}
function hashToScalar(bytes) {
  const hash = (0, import_sha2562.sha256)(bytes);
  const num = bytesToNumber(hash);
  return num % import_p2562.p256.CURVE.n;
}
...
function domainHex(domain, value) {
  return hex((0, import_sha2563.sha256)(concatBytes3(ascii(domain), value)));
}
...
async function validateExchangeDiscovery(issuerId, discovery) {
  ...
  for (let graphIndex = 0; graphIndex < graphs.length; graphIndex++) {
    ...
    for (const descriptor of graph.descriptors) {
      ...
      if (hex((0, import_sha2564.sha256)(spki)) !== descriptor.token_key_id)
        invalid();
...
function validateReceiptDiscoveryKey(key, purpose, ids) {
  ...
  if (!import_ed25519.ed25519.utils.isValidPublicKey(publicKey, false) || hex((0, import_sha2564.sha256)(publicKey)) !== key.key_id) {
    throw new DiscoveryError("Invalid V2 exchange discovery metadata");
  }
  ids.add(key.key_id);
}
...
function exchangeRequestDigest(request) {
  return bytesToBase64Url((0, import_sha2565.sha256)(new Uint8Array([
...
async function isExchangeSuccessResponse(state, value, submittedRequest, selectedGraph) {
  ...
  const calculatedResultDigest = bytesToBase64Url(
    (0, import_sha2565.sha256)(new Uint8Array([
  ...
  if (!receiptKey || receipt.created_at < receiptKey.valid_from || receipt.expires_at > receiptKey.valid_until)
    return false;
  const receiptDigest = (0, import_sha2565.sha256)(new Uint8Array([
...
function graphIssuanceRequestDigest(request) {
  return bytesToBase64Url((0, import_sha2566.sha256)(new Uint8Array([
...
function isGraphIssuanceResult(value, request, expectedTokenKeyId, digest) {
  ...
  if (value.request_digest !== digest(request))
    return false;
  const resultDigest = bytesToBase64Url((0, import_sha2566.sha256)(new Uint8Array([
...
// src/crypto/rsa.ts
var import_blindrsa_ts = require("@cloudflare/blindrsa-ts");
var suite = import_blindrsa_ts.RSABSSA.SHA384.PSS.Deterministic();
...
function hashPow(input, nonce, timestamp) {
  return (0, import_sha2567.sha256)(concatBytes3(
...
function buildBatchBinding(routeScope, issuerId, blindedElements) {
  const hasher = import_sha2567.sha256.create();
...
function graphIssuanceHmacAuthorizationTagV2(secret, nonce, policyId, authorizationBindingDigest) {
  ...
  return (0, import_hmac.hmac)(import_sha2568.sha256, secret, graphIssuanceHmacAuthorizationTranscriptV2(
    nonce,
    policyId,
    authorizationBindingDigest
  ));
